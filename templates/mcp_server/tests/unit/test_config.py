...
from mcp_server.config import register_configs


def _compose_config(overrides: list[str] | None = None) -> DictConfig:
    register_configs()
    with initialize_config_module(version_base=None, config_module="mcp_server.conf"):
...
def test_compose_config_returns_hydra_config() -> None:
    ...
    assert cfg.server.name == "mcp-server"
...
def test_hydra_config_preserves_lazy_interpolations() -> None:
    cfg = _compose_config(["hello.default_name=${server.name}"])

    assert cfg.server.name == "mcp-server"
    assert cfg.hello.default_name == "mcp-server"
